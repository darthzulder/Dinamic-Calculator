...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
            ...
            ScaleGestureDetector(
                    ...
                    object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
                        ...
                        override fun onScale(detector: ScaleGestureDetector): Boolean {
                            ...
                            // Matemáticas para hacer zoom hacia el punto focal (donde están los
                            // dedos)
                            if (scaleFactor != prevScale) {
                                val focusX = detector.focusX
                                val focusY = detector.focusY

                                // Ajustamos la posición para que el punto bajo el foco se mantenga
                                // estable
                                // Fórmula: NuevaPos = Foco - (Foco - ViejaPos) * (NuevoScale /
                                // ViejoScale)
            ...
            GestureDetector(
                    ...
                    object : GestureDetector.SimpleOnGestureListener() {
                        override fun onDoubleTap(e: MotionEvent): Boolean {
                            ...
                            val scaleChange = targetScale / prevScale
                            posX = e.x - (e.x - posX) * scaleChange
                            posY = e.y - (e.y - posY) * scaleChange
    ...
    private fun isTouchingNode(x: Float, y: Float): Boolean {
        ...
        for (i in container.childCount - 1 downTo 0) {
            ...
            if (child.visibility == View.VISIBLE &&
                            child.id != com.dz.calculator.R.id.connection_lines_view
            ) {
                ...
                if (localPoint.x >= child.x &&
                                localPoint.x <= child.x + child.width &&
                                localPoint.y >= child.y &&
                                localPoint.y <= child.y + child.height
                ) {
                    return true
                }
            }
    ...
    /**
     * Expande el canvas_container para que pueda contener todos los nodos, incluso cuando están
     * fuera del área visible original. El canvas inicia con el tamaño de 2x la pantalla y crece
     * dinámicamente. Esto permite que el drag and drop funcione en todo el espacio del canvas.
     */
    fun expandCanvasForNodes(nodes: List<com.dz.calculator.canvas.CalculationNode>) {
        ...
        if (nodes.isEmpty()) {
            ...
            // Solo aplicar si aún no tiene el tamaño correcto (para no invalidar layout
            // innecesariamente)
            if (layoutParams.width < minCanvasWidth.toInt() ||
        ...
        // Calcular el tamaño necesario considerando que los nodos pueden estar en cualquier
        // posición
        val requiredWidth = maxOf(minCanvasWidth, maxX + padding, -minX + minCanvasWidth + padding)
        val requiredHeight =
                maxOf(minCanvasHeight, maxY + padding, -minY + minCanvasHeight + padding)
        ...
        // Solo actualizar si el tamaño cambió significativamente (evitar actualizaciones
        // innecesarias)
    ...
    fun screenToCanvas(screenX: Float, screenY: Float): PointF {
        ...
        return PointF((screenX - posX) / scaleFactor, (screenY - posY) / scaleFactor)
    }

    /**
     * Ajusta la posición de paneo (posX, posY) para compensar un desplazamiento de los nodos. Se
     * usa cuando los nodos se desplazan (shift) para mantener coordenadas positivas.
